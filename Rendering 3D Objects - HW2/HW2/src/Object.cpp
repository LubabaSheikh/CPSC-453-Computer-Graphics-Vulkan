...
struct ObjectPushConstants {
	...
	float global_angleX = 0;
    float global_angleY = 0;
    float global_angleZ = 0;
	float local_angleX = 0;
    float local_angleY = 0;
    float local_angleZ = 0;
...
void objectCreateGeometryAndBuffers() 
{
	...
	*/
	...
	// Pre-process teapot
	preProcessingObjects(positions, indices, vertexNormals);

	// Calculate Normals and Normalize
	calculateNormalsAndNormalize(positions, indices, vertexNormals);
	...
	for( unsigned int i = 0; i < vData.size(); i++ ) {
		...
		vData[i].normal = vertexNormals[i]; //glm::vec3( dis(gen), dis(gen), dis(gen) );
	}
...
void objectCreatePipeline() {
	...
	// Set light position
	pushConstants.lightPos = glm::vec3(20.0, 20.0, 5.0);
...
void objectUpdateConstants() {
    ...
    pushConstants.model = pushConstants.model * glm::scale(glm::mat4(1.0f), glm::vec3(pushConstants.scale, pushConstants.scale, pushConstants.scale));
    
	// Extrinsic Rotation Matrix
	auto extrinsicRotation = glm::rotate(glm::mat4(1.0f), pushConstants.global_angleX, glm::vec3(1.0f, 0.0f, 0.0f)) *
							 glm::rotate(glm::mat4(1.0f), pushConstants.global_angleY, glm::vec3(0.0f, 1.0f, 0.0f)) *
							 glm::rotate(glm::mat4(1.0f), pushConstants.global_angleZ, glm::vec3(0.0f, 0.0f, 1.0f));

	// Extrinsic Rotation Matrix
	auto intrinsicRotation = glm::rotate(glm::mat4(1.0f), pushConstants.local_angleX, glm::vec3(1.0f, 0.0f, 0.0f)) *
							 glm::rotate(glm::mat4(1.0f), pushConstants.local_angleY, glm::vec3(0.0f, 1.0f, 0.0f)) *
							 glm::rotate(glm::mat4(1.0f), pushConstants.local_angleZ, glm::vec3(0.0f, 0.0f, 1.0f));

    // Applying Intrinsic Rotations
	pushConstants.model = pushConstants.model * intrinsicRotation;

	// Applying Extrinsic Rotation
	pushConstants.model = extrinsicRotation * pushConstants.model;

	// Set scales and angles back to 0
	pushConstants.global_angleX = 0.0;
	pushConstants.global_angleY = 0.0;
	pushConstants.global_angleZ = 0.0;
	pushConstants.local_angleX = 0.0;
	pushConstants.local_angleY = 0.0;
	pushConstants.local_angleZ = 0.0;
	pushConstants.scale = 1.0;
...
void updateViewAndProjectionMatrix(VklCameraHandle camera)
{
	...
	pushConstants.cameraPosition = vklGetCameraPosition(camera);
}

void scaleUp()
{
	pushConstants.scale += 0.1;
}
...
{
	pushConstants.global_angleX += angle;
}

void rotateYExtrinsic(float angle)
{
	pushConstants.global_angleY += angle;
}

void rotateZExtrinsic(float angle)
{
	pushConstants.global_angleZ += angle;
}

void rotateXIntrinsic(float angle)
{
	pushConstants.local_angleX += angle;
}

void rotateYIntrinsic(float angle)
{
	pushConstants.local_angleY += angle;
}

void rotateZIntrinsic(float angle)
{
	pushConstants.local_angleZ += angle;
}

void drawObjects(std::string filepath)
{
	auto model = vklLoadModelGeometry(filepath);

	positions = model.positions;
	indices = model.indices; // Object indices of triangles 

	std::vector<glm::vec3> vertexNormals(positions.size(), glm::vec3(0.0f));
	
	// Pre-process images to fit screen
	preProcessingObjects(positions, indices, vertexNormals);
	...
	for( unsigned int i = 0; i < vData.size(); i++ ) {
		vData[i].position = positions[i];
		vData[i].normal = vertexNormals[i];
	}
...
void preProcessingObjects(std::vector<glm::vec3> &positions, std::vector<unsigned int> &indices, std::vector<glm::vec3> &vertexNormals)
{
	...
	// Calculate the min and max for x, y and z
    ...
    float objectCenterX = (minX + maxX) * 0.5f;
    float objectCenterY = (minY + maxY) * 0.5f;
    float objectCenterZ = (minZ + maxZ) * 0.5f;
    // Calculate the translation vector to center the object
    float translationX = -objectCenterX;
    float translationY = -objectCenterY;
    float translationZ = -objectCenterZ;
    // Apply the translation to the object's vertices
	...
	// Calculate the maximum absolute dimension
    float maxDimension = glm::length(maxX - minX);
    // Determine the desired size within the range of -1 to 1
    float desiredSize = 2.0f;  // For the range -1 to 1

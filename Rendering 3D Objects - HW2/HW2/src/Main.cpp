...
int main(int argc, char** argv)
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		updateViewAndProjectionMatrix(camera);
...
void handleGlfwKeyCallback(GLFWwindow* glfw_window, int key, int scancode, int action, int mods) 
{
	...
	if (intrinsicFlag == true) // Do intrinsic rotations
	{
		// X-Axis Rotation
		if( key == GLFW_KEY_RIGHT && action == GLFW_REPEAT ) {
			rotateXIntrinsic(0.1);
		}
		...
		if( key == GLFW_KEY_D && action == GLFW_REPEAT ) {
			rotateZIntrinsic(0.1);
		}
		if( key == GLFW_KEY_A && action == GLFW_REPEAT ) {
			rotateZIntrinsic(-0.1);
		}
	}
	if (intrinsicFlag == false) // Do extrinsic rotations
	{
		// X-Axis Rotation
		if( key == GLFW_KEY_RIGHT && action == GLFW_REPEAT ) {
			rotateXExtrinsic(0.1);
		}
		if( key == GLFW_KEY_LEFT && action == GLFW_REPEAT ) {
			rotateXExtrinsic(-0.1);
		}
    ...
    {
		drawObjects("../HW2/models/airplane.obj");
	}
	// Load diamond model
	if( key == GLFW_KEY_2 && action == GLFW_PRESS)
    {
		drawObjects("../HW2/models/diamond.obj");
	}
	// Load ducky model
	if( key == GLFW_KEY_3 && action == GLFW_PRESS)
    {
		drawObjects("../HW2/models/ducky.obj");
	}
	// Load hello model
	if( key == GLFW_KEY_4 && action == GLFW_PRESS)
    {
		drawObjects("../HW2/models/hello.obj");
	}
	// Load mug model
	if( key == GLFW_KEY_5 && action == GLFW_PRESS)
    {
		drawObjects("../HW2/models/mug.obj");
	}
	// Load Nefertiti model
	if( key == GLFW_KEY_6 && action == GLFW_PRESS)
    {
		drawObjects("../HW2/models/Nefertiti.obj");
	}

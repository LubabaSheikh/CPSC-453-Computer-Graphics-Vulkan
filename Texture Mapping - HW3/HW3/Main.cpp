...
int main(int argc, char** argv)
{
	...
	glfwSetWindowSizeCallback(window, handleResizeCallback);
	{
		...
		// std::string imagePath = shared::Path::Instance->Get("models/chess_bishop/bishop.colour.black.png");
		
		if(argc != 4)
		{
			VKL_LOG("Must enter 3 arguments in the command line: ./HW3 'object file path' 'color file path' 'ambient occlusion file path'");
			return -1;
		}
		...
		// Ambient Occlusion
		// std::string aoPath = shared::Path::Instance->Get("models/chess_bishop/bishop.ao.png");
		std::string aoPath = argv[3];
		// std::string aoPath = shared::Path::Instance->Get("models/timmy_cup/timmy_cup.ao.png");

		// Now create initial geometry and pass it to the GPU
		// Obj file is expected to be passed as a command line arg
		objectCreateGeometryAndBuffers(modelPath, window, vk_queue, imagePath, aoPath);

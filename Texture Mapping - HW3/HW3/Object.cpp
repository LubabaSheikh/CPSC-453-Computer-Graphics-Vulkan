...
std::array<VkDescriptorSetLayoutBinding, 4> allbindings;
...
VkDescriptorSetLayout aodescriptorSetLayout;

// std::array<VkDescriptorSetLayoutBinding, 1> aobindings;

VkDeviceMemory pntextureImageMemory;
VkImage pntextureImage;
VkImageView pntextureImageView;
VkSampler pntextureSampler;
VkDescriptorSetLayout pndescriptorSetLayout;
const int gridSize = 16;
//Create a vector of vectors to represent the grid
std::vector<std::vector<int>> grid(gridSize, std::vector<int>(gridSize, 0)); // Initialized with zeros

// auto aoModeBuf = vklCreateHostCoherentBufferWithBackingMemory(
//             sizeof(int), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
...
void objectCreateGeometryAndBuffers( const std::string& path_to_obj, GLFWwindow* window, VkQueue vkQueue, const std::string& imagePath, const std::string& aoPath) 
{
	...
	// CALL CREATING TEXTURE, IMAGEVIEW, SAMPLER HERE
	textureImage = createTextureImage(imagePath, vkQueue);
	textureImageView = createTextureImageView(textureImage);
	textureSampler = createTextureSampler();
	
	// AO
	aotextureImage = createTextureImage(aoPath, vkQueue);
	aotextureImageView = createTextureImageView(aotextureImage);
	aotextureSampler = createTextureSampler();

	// PN
	grid = createAndInitializeGrid(gridSize);
	pntextureImage = createTextureImagefor2DGrid(grid, vkQueue);
	pntextureImageView = createTextureImageView(pntextureImage);
	pntextureSampler = createTextureSampler();
	
	// COMBINE EVERYTHING
	descriptorSetLayout = createDescriptorSetLayout();
	allbindings = createBindings(descriptorSetLayout);
	descriptorPool = createDescriptorPool(); //"t");
	descriptorSet = createDescriptorSets(descriptorSetLayout, textureImageView, textureSampler, 
		aotextureImageView, aotextureSampler, pntextureImageView, pntextureSampler, descriptorPool);
...
void objectCreatePipeline() {
	...
	VklGraphicsPipelineConfig config{};
		...
		// NEED THIS
		config.descriptorLayout.emplace_back(allbindings[0]);
		config.descriptorLayout.emplace_back(allbindings[1]);
		config.descriptorLayout.emplace_back(allbindings[2]);
		config.descriptorLayout.emplace_back(allbindings[3]);
...
void objectUpdateConstants( GLFWwindow* window ) {
	...
	// and scale
	pushConstants.aoState = aoConfig;
	pushConstants.pnState = pnConfig;
	pushConstants.texState = texConfig;
	pushConstants.mState = mConfig;
